...
--[[LAYOUT
	...
	["Name Of Store"]=
	{
		...
		x=-706.03, y=-915.42, z=19.21, 	-- THE POSITION OF THE PLACE
		...
		type = 'Register',				-- Robbery type used for custom drop addon
		chance = 10						-- How much % that item is dropped (works only if global settings are disabled)
	}

]]

--[[
	CUSTOM ITEM DROP DOCUMENTATION

	Possible Robbery Types:
...
local enabled =				-- Defines which robbery types can drop items
{
	['Register'] = false,
	['Safe'] 	 = true,
	['Bank'] 	 = true,
...
local globalSettings =	-- Used to define global settings for item drop chance
{
	['Register'] = 	{ enabled = true, chance = 75 },
	['Safe'] = 		{ enabled = true, chance = 10 },
	['Bank'] = 		{ enabled = false, chance = 10 },
	['Pacific'] = 	{ enabled = false, chance = 10 }
}
local itemDrop =		-- Defines which robbery type drops what item
{
	['Register'] =  {name='id_black', label='Bank Truck ID'},
	['Safe'] = 		{name='id_green', label='Fleeca ID'},
	['Bank'] =		{name='id_gold',  label='Pacific ID'},
	['Pacific'] = 	{}
}
...
--	{
--		name="West Hawick Flecca Bank Vault",         -- Del Perro     ---This is another fleeca circle. WTF?
--		blip=500,
--		blipColor=6,
--		blipSize=0.8,
--		x=-1206.41, y=-337.71, z=37.76,
--		beingRobbed=false,
--		timeToRob = 900,
--		isSafe=true,
--		copsNeeded = 4,
...
--	["Hawick Flecca Bank Vault"]=             -- Is this an actual bank?
--	{
--		name="Hawick Flecca Bank Vault",
--		blip=500,
--		blipColor=6,
--		blipSize=0.8,
--		x=-351.60, y=-59.34, z=49.01,
--		beingRobbed=false,
--		timeToRob = 900,
--		isSafe=true,
--		copsNeeded = 4,
			...
			if(Vdist(x,y,z,robbableSpot.x,robbableSpot.y,robbableSpot.z)<distanceForMarkerToShow and isRobbing==false and robbableSpot.beingRobbed==false)then
				...
				if(Vdist(x,y,z,robbableSpot.x,robbableSpot.y,robbableSpot.z)<distanceToInteractWithMarker)then
					...
					if (IsControlJustPressed(1, keyToInteractWithRobbery)) then
						if robbableSpot.type == 'Bank' then TriggerServerEvent('robberies:doesHaveItem', 'green_card', 'Fleeca ID')
						elseif robbableSpot.type == 'Pacific' then TriggerServerEvent('robberies:doesHaveItem', 'gold_card', 'Pacific ID')
						else TriggerServerEvent("robberies:policeCheck") end
					end
				else
					if(isRobbing)then
						StopRobbery()
...
AddEventHandler("robberies:StartRobbery", function(cops)
	...
	Citizen.CreateThread(function()
		if(cops>=spotBeingRobbed.copsNeeded)then
			...
			TriggerServerEvent("robberies:SendNotiToCops", "10-90 : A silent alarm has been triggered ( "..spotBeingRobbed.name.." )", {x = spotBeingRobbed.x, y = spotBeingRobbed.y, z = spotBeingRobbed.z})
			Citizen.CreateThread(function()
				while isRobbing do
					exports['drp_progressBars']:startUI(spotBeingRobbed.timeToRob, "Doing Something")
					if(spotBeingRobbed.isSafe)then
						...
						exports['drp_progressBars']:startUI(spotBeingRobbed.timeToRob-currentSecondCount, "Doing Something")
					else
						exports['drp_progressBars']:startUI(spotBeingRobbed.timeToRob-currentSecondCount, "Doing Something")
...
function RobberyOver(spotBeingRobbed)
	DisplayHelpText("You have sucesffully robbed this spot!")
	local received = false
	if enabled[spotBeingRobbed.type] then
		if globalSettings[spotBeingRobbed.type].enabled then received = math.random(0, 100) <= globalSettings[spotBeingRobbed.type].chance
		else received = math.random(0, 100) <= spotBeingRobbed.chance end
		if received then
			TriggerServerEvent('robberies:giveItem', itemDrop[spotBeingRobbed.type].name)
			TriggerEvent("pNotify:SendNotification", {
				text = "You received "..itemDrop[spotBeingRobbed.type].label,
				timeout = 3000,
				layout = "centerRight",
				queue = "right"
			})
...
AddEventHandler("robberies:robberyOverNotification", function(name)
	local location = robbableSpots[name]
	TriggerServerEvent("robberies:SendNotiToCopsOver", "10-90 : Somebody was seen fleeing the area ( "..location.name.." )", {x = location.x, y = location.y, z = location.z})
end)

RegisterNetEvent("robberies:robberyStartedNotification")
AddEventHandler("robberies:robberyStartedNotification", function(name)
	local location = robbableSpots[name]
	TriggerServerEvent("robberies:SendNotiToCopsStart", "10-90 : A silent alarm has been triggered ( "..location.name.." )", {x = location.x, y = location.y, z = location.z})
end)
...
RegisterNetEvent('robberies:hasItem')
AddEventHandler('robberies:hasItem', function()
